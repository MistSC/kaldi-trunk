...
parser.add_option('--lastlayer-type', dest='lastlayer_type',
                  help='Select last layer activation function type : (<Sigmoid> [default: %default])',
...
if num_hid_layers == 0 and o.bottleneck_dim != 0:
  ...
  if o.with_softmax:
    if o.block_softmax_dims == "":
      print "<%s> <InputDim> %d <OutputDim> %d" % (o.lastlayer_type, num_leaves, num_leaves)
    else:
...
if num_hid_layers == 0:
  ...
  if o.with_softmax:
    if o.block_softmax_dims == "":
      print "<%s> <InputDim> %d <OutputDim> %d" % (o.lastlayer_type ,num_leaves, num_leaves)          #here has change
    else:
      print "<BlockSoftmax> <InputDim> %d <OutputDim> %d <BlockDims> %s" % (num_leaves, num_leaves, o.block_softmax_dims)
  print "</NnetProto>"
  # We are done!
