...
static void Test1()
{
  CuMatrix<Real> m1(100,500);
  CuMatrix<Real> m2(500,100);
  CuMatrix<Real> m3(100,100);
  m1.Set(1);
  m2.Set(2);
  m3.SetZero();
  auto up = std::chrono::steady_clock::now();
  m3.AddMatMat(1.0,m1,kNoTrans,m2,kNoTrans,1.0);
  auto down = std::chrono::steady_clock::now();
  auto d = std::chrono::duration_cast<std::chrono::microseconds>(down - up);
  KALDI_LOG<< "==================time:" <<d.count() <<"ms";
}

template<typename Real>
static void Test2()
{
  CuMatrix<Real> m1(1000,1000);
  CuMatrix<Real> m2(1000,1000);
  CuMatrix<Real> m3(1000,1000);
  m1.Set(1);
  m2.Set(2);
  m3.SetZero();
  auto up = std::chrono::steady_clock::now();
  m3.AddMatMatElements(1.0,m1,m2,1.0);
  auto down = std::chrono::steady_clock::now();
  auto d = std::chrono::duration_cast<std::chrono::microseconds>(down - up);
  KALDI_LOG<< "==================time:" <<d.count() <<"ms";
}
...
static void Test3()
{
  CuMatrix<Real> m1(200,1000);
  CuMatrix<Real> m2(100,10);
  CuMatrix<Real> m3(200,100);
  m1.Set(1);
  m2.Set(2);
  m3.SetZero();
  std::vector< CuSubMatrix<Real>* > vec_m1;
  std::vector< CuSubMatrix<Real>* > vec_m2;
  std::vector< CuSubMatrix<Real>* > vec_m3;
  CuSubMatrix<Real>* sw2 = new CuSubMatrix<Real>(m2,0,100,0,10);
  for(int32 i=0;i<200;i++)
  {
    CuSubMatrix<Real>* temp_sm1 = new CuSubMatrix<Real>(m1,i,10,100);
    CuSubMatrix<Real>* temp_sm3 = new CuSubMatrix<Real>(m3,i,10,10);
    vec_m1.push_back(temp_sm1);
    vec_m2.push_back(sw2);
    vec_m3.push_back(temp_sm3);
  }
  auto up = std::chrono::steady_clock::now();
  AddMatMatBatched<Real>(1.0,vec_m3,vec_m1,kNoTrans,vec_m2,kNoTrans,0.0);
  auto down = std::chrono::steady_clock::now();
  auto d = std::chrono::duration_cast<std::chrono::microseconds>(down - up);
  KALDI_LOG<< "==================time:" <<d.count() <<"ms";
  for(auto i=vec_m1.begin(); i!=vec_m1.end(); ++i)
    delete *i;
  for(auto i=vec_m3.begin(); i!=vec_m3.end(); ++i)
    delete *i;
  delete sw2;
}
...
static void Test4()
{
  CuMatrix<Real> m1(10,100);
  CuMatrix<Real> m2(100,150);
  CuMatrix<Real> m3(100,150);
  
  auto up = std::chrono::steady_clock::now();
  m3.AddVecMatPointMulVec(1.0,m1,m2,0.0,0);
  auto down = std::chrono::steady_clock::now();
  auto d = std::chrono::duration_cast<std::chrono::microseconds>(down - up);
  KALDI_LOG<< "==================time:" <<d.count() <<"ms";
}
