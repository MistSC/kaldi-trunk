...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    po.Register("is-binary", &is_binary, "save each layer use binary or not");

    std::string frame_index = "-1";
    po.Register("frame-index", &frame_index, "frame index for storage");
    std::string file_name = "";
    po.Register("file-name", &file_name, "name of file for storage each layer features");
    ...
    int32 frame_i;
    ConvertStringToInteger<int32>(frame_index,&frame_i);
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      if (index==frame_i)
      {
        nnet.Feedforward(feats_transf, &nnet_out, is_binary, file_name);
      }
      else
      {
