...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        ...
        "\n"
        "Usage:  nnet-forward [options] <model-in> <feature-rspecifier> <feature-wspecifier> <frame-index> <frame-length> <model-out>\n"
        "e.g.: \n"
    ...
    po.Register("is-binary", &is_binary, "save each layer use binary");
    ...
    int32 frame_index, frame_length;
    ConvertStringToInteger<int32>(po.GetArg(4),&frame_index);
    ConvertStringToInteger<int32>(po.GetArg(5),&frame_length);
    std::vector<std::string> output_filename;
    for(int32 i=6;i<=po.NumArgs();i++)
    {
        output_filename.push_back(po.GetArg(i));
    }   
    
    // delete some impossibe choices
    if (frame_length!=output_filename.size())
    {
      po.PrintUsage();
      exit(1);
    }
    ...
    double time_now = 0;
    int32 num_done = 0;
    int32 index = 0;// this val used for controling the Feedforward
    std::vector<std::string>::iterator it = output_filename.begin();
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      // fwd-pass, nnet,
      index++;
      if (index>=frame_index && index<=frame_index+frame_length)
      {
        nnet.Feedforward(feats_transf, &nnet_out, false, *it++);
      }
      else
      {
