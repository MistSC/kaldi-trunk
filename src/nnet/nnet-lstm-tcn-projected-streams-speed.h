...
class LstmTcnProjectedStreamsSpeed : public UpdatableComponent {
 public:
  ...
  void InitData(std::istream &is) {
    ...
    w_ifo_x_.Resize(3*ncell_, input_dim_, kUndefined);
    // w_r: ncell_ * nrecur_
    ...
    InitMatParam(w_ifo_x_, param_scale);
  ...
  void ReadData(std::istream &is, bool binary) {
    ...
    w_ifo_x_corr_.Resize(3*ncell_, input_dim_, kSetZero);
  ...
  int32 NumParams() const {
    return ( // w_gifo_x_.NumRows() * w_gifo_x_.NumCols() +
         ...
         w_ifo_x_.NumRows() * w_ifo_x_.NumCols() +
         w_gifo_r_.NumRows() * w_gifo_r_.NumCols() +
         bias_.Dim() +
         peephole_i_c_.Dim() +
         peephole_f_c_.Dim() +
  ...
  void GetParams(Vector<BaseFloat>* wei_copy) const {
    ...
    offset += len; len = w_ifo_x_.NumRows() * w_ifo_x_.NumCols();
    wei_copy->Range(offset, len).CopyRowsFromMat(w_ifo_x_);
  ...
  std::string Info() const {
    return std::string("  ") +
      ...
      "\n  w_ifo_x_  "   + MomentStatistics(w_ifo_x_) +
  ...
  std::string InfoGradient() const {
    ...
    return std::string("  ") +
      ...
      "\n  w_ifo_x_corr_  "      + MomentStatistics(w_ifo_x_corr_) +
  ...
  void PropagateFnc(const CuMatrixBase<BaseFloat> &in, CuMatrixBase<BaseFloat> *out) {
    ...
    CuSubMatrix<BaseFloat> YIFO(propagate_buf_.ColRange(0, 3*ncell_));
    ...
    TuckerFeedForward(w_g_x_mode_1_, w_g_x_mode_2_,
                      in, &YG_tt);
    // y -> i, f, o
    YIFO.RowRange(1*S,T*S).AddMatMat(1.0, in, kNoTrans, w_ifo_x_, kTrans, 0.0);
    ...
    for (int t = 1; t <= T; t++) {
      ...
      CuSubMatrix<BaseFloat> y_ifo(YIFO.RowRange(t*S,S));
      /// r(t-1) -> g, i, f, o
  ...
  void BackpropagateFnc(const CuMatrixBase<BaseFloat> &in, const CuMatrixBase<BaseFloat> &out,
    ...
    CuSubMatrix<BaseFloat> DIFO(backpropagate_buf_.ColRange(0, 3*ncell_));
    ...
    //KALDI_LOG<<"g,i,f,o -> x, do it all in once";
    ...
    // indiff_tmp.SetZero();
    //KALDI_LOG<<"in_diff:("<<in_diff->NumRows()<<","<<in_diff->NumCols()<<")";
    //KALDI_LOG<<"indiff_tmp:("<<indiff_tmp.NumRows()<<","<<indiff_tmp.NumCols()<<")";
    // g -> x 
    ...
    // indiff_tmp.SetZero();
    // i, f, o -> x
    in_diff->AddMatMat(1.0, DIFO.RowRange(1*S,T*S), kNoTrans, w_ifo_x_, kNoTrans, 0.0);
    ...
    TuckerCalcGrad(w_g_x_mode_1_, w_g_x_mode_2_,
                   ...
                   mmt);
    // x -> i, f, o
    w_ifo_x_corr_.AddMatMat(1.0, DIFO.RowRange(1*S,T*S), kTrans, in, kNoTrans, mmt);
    ...
    if (clip_gradient_ > 0.0) {
      ...
      */
      ...
      w_ifo_x_corr_.ApplyFloor(-clip_gradient_);
      w_ifo_x_corr_.ApplyCeiling(clip_gradient_);
    ...
    if (DEBUG) {
      ...
      std::cerr << "w_ifo_x_corr_ " << w_ifo_x_corr_;
  ...
  void Update(const CuMatrixBase<BaseFloat> &input, const CuMatrixBase<BaseFloat> &diff) {
    ...
    w_ifo_x_.AddMat(-lr, w_ifo_x_corr_);
...
//
 ...
 private:
  ...
  CuMatrix<BaseFloat> w_ifo_x_;
  CuMatrix<BaseFloat> w_ifo_x_corr_;
